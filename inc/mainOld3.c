...
void drawOLEDX(uint16_t light_val, int32_t temp_val){
	...
	oled_putString(1, 0, printLight, OLED_COLOR_BLACK, OLED_COLOR_WHITE);
	oled_putString(1,10, printTemp, OLED_COLOR_BLACK, OLED_COLOR_WHITE);
	oled_putString(1,20, printAcc0, OLED_COLOR_BLACK, OLED_COLOR_WHITE);
	oled_putString(1,30, printAcc1, OLED_COLOR_BLACK, OLED_COLOR_WHITE);
	Timer0_Wait(1);
}
...
int main (void) {
    ...
    while (1)
    {
        ...
        if(is1000){
    		...
    		increment7Seg();
    		is1000 = 0;
    		//oled_fillRect(10, 10, 40, 40, OLED_COLOR_WHITE);
        }
